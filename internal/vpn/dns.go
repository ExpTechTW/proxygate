...
// redirects traffic, and it prefers UDP. These DNS-over-TCP A queries use only
// literal IPv4 endpoints carried by the VPN session.
func resolveSessionIPv4(ctx context.Context, session Session, host string, servers []string) ([]net.IP, error) {
	...
	return nil, fmt.Errorf("IPv4 TCP DNS query for %s through VPN: %w", host, errors.Join(failures...))
}
...
func querySessionA(ctx context.Context, session Session, host, server string) ([]net.IP, error) {
	...
	connection, err := session.DialContext(ctx, "tcp4", server)
	if err != nil {
		return nil, fmt.Errorf("connect TCP DNS: %w", err)
	}
	...
	for len(frame) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("write TCP DNS query: %w", err)
		}
		if written == 0 {
	...
	if _, err := io.ReadFull(connection, length[:]); err != nil {
		return nil, fmt.Errorf("read TCP DNS length: %w", err)
	}
	responsePayload := make([]byte, int(binary.BigEndian.Uint16(length[:])))
	if len(responsePayload) < 12 {
		return nil, errors.New("TCP DNS response is too short")
	}
	if _, err := io.ReadFull(connection, responsePayload); err != nil {
		return nil, fmt.Errorf("read TCP DNS response: %w", err)
	}
	var response dnsmessage.Message
	if err := response.Unpack(responsePayload); err != nil {
		return nil, fmt.Errorf("decode TCP DNS response: %w", err)
	}
	if !response.Header.Response || response.Header.ID != id {
		return nil, errors.New("TCP DNS response does not match query")
	}
	if len(response.Questions) != 1 || response.Questions[0].Type != dnsmessage.TypeA || response.Questions[0].Name.String() != name.String() {
		return nil, errors.New("TCP DNS response contains an unexpected question")
	}
	if response.Header.RCode != dnsmessage.RCodeSuccess {
		return nil, fmt.Errorf("TCP DNS returned %s", response.Header.RCode)
	}
